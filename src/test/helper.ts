...
const sessionSchema = new Schema({
  name: hdbStoreOptions.schema, fields: {session:'',expiration:''}, silent: true, primaryKey: 'sid'
});
...
export const storeCreator = (options: Partial<HarperDBStoreOptions> = {}) => {
  ...
  const asyncStore = {
    ...
    clear: 
    promisify(store.clear).bind(store),
    get: 
    promisify(store.get).bind(store),
    set: 
    promisify(store.set).bind(store),
    all: 
    promisify(store.all).bind(store),
    touch: 
    promisify(store.touch).bind(store),
    destroy: 
    promisify(store.destroy).bind(store),
